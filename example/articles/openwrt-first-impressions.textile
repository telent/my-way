...
or editorial control.  This isn't listed first because it's most
...
fussy about *which* files. If there's a file under /etc/config with a
relevant-looking setting in it, edit that in preference to whatever
the upstream app's usual config file would be, then run @uci commit@ -
although actually you might not need to run @uci commit@ - see 
"this lovely thread":https://forum.openwrt.org/viewtopic.php?id=30428 
for the full confusing detail - 
then run the relevant @/etc/init.d/foo@ scripts to
restart services as needed.  I am not sure if there's a clear rule for
what overrides what and what clobbers what if you edit config files
...
Summary: it's not currently suitable for the non-technical end user,
but if you have some Linux experience and a few hours to screw around
